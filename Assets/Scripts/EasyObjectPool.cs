...
namespace MarchingBytes
{
    ...
    class Pool
    {
        ...
        public Pool(string poolName, GameObject poolObjectPrefab, int initialCount, bool fixedSize, Transform transformParent)
        {
            ...
            {
                AddObjectToPool(NewObjectInstance(transformParent));
            }
        }
        ...
        //o(1)
        public GameObject NextAvailableObject(Vector3 position, Quaternion rotation, Transform transformParent)
        {
            PoolObject po = null;
    ...
    /// Easy object pool.
    /// </summary>
    public class EasyObjectPool : Singleton<EasyObjectPool>
    {
		...
		*/
        public GameObject GetObjectFromPool(string poolName, Vector3 position, Quaternion rotation)
        {
            ...
            if (poolDictionary.ContainsKey(poolName))
            {
                ...
                result = pool.NextAvailableObject(position, rotation, transform);
